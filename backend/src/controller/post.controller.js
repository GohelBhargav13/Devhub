...
export const newPostCreation = async(req,res) => {
    try {
        const { post_description,post_link,post_tags } = req.body

        if(!post_description || !post_link || !post_tags){
            return res.status(400).json({ StatusCode:400, message: "All fields are required" })
        }

        const user_id = req.user.user_id
        const {status,new_post} = await addNewPost(post_description,post_link,user_id,post_tags)
