...
import ActiveUserDeatils from './ActiveUserDeatils.jsx'
...
const DashBoard = () => {
    ...
    const [totalActiveUserDeatils,setActiveUserDetails] = useState([])
       ...
       const fetchAllCounts = async() => {
          try {
            ...
            if(t_c?.status){
              ...
              setActiveUserDetails(total_active_users?.data?.active_user_details)
            }
          } catch (error) {
            console.log("Error while fetch all users count and details ",error.message)
            setIsLoading(false)
          }finally{
              setIsLoading(false)
          }
  ...
  return (
   <div className='flex flex-row gap-5'>
        ...
        <div className='flex flex-col gap-5 py-10 w-full h-auto'>
          ...
          <div className='grid grid-cols-2 w-full h-50 gap-5 p-4'>
                <div className='bg-linear-to-br from-slate-800 via-slate-700 to-slate-900 p-3 rounded-xl border-r-2 border-b-4 border-white border-r-cyan-300 text-center'>
                    <div className='w-full h-auto p-8 bg-slate-800 rounded-3xl border-t-2 border-l-4 border-white border-l-cyan-300'>
                        ...
                        { isLoading ? (
                          <Loader2 className='animate-spin duration-300 flex justify-center w-full mt-4' size={30} />
                 ...
                 <div className='bg-linear-to-br from-slate-800 via-slate-700 to-slate-900 p-3 rounded-xl border-r-2 border-b-4 border-white border-r-cyan-300 text-center'>
                       <div className='w-full h-auto p-8 bg-slate-800 rounded-3xl border-t-2 border-l-4 border-white border-l-cyan-300'>
                        <p className='font-bold font-mono text-[22px]'>Active User Deatils:</p>
                        { isLoading ? (
                          <Loader2 className='animate-spin duration-300 flex justify-center w-full mt-4' size={30} />
                        ): (
                           <a href='#activeusers' className='text-[30px] text-blue-600 underline cursor-pointer'>{ total_active }+</a>
                        )}
                    </div>
                </div>
          </div>
          <section id='activeusers'>
            <div className='mt-50 text-center'>
              <p className='font-bold text-[22px] font-mono mr-2 border-b-4 border-b-white rounded-xl'> Current Active Users </p>
              <ActiveUserDeatils activeUsers={totalActiveUserDeatils} />
            </div>
          </section>
        </div>
   </div>
